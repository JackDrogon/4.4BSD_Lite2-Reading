 ...
 */
tmscpopen(io) register struct iob *io;
{
	...
	if (tmscp_offline) {
		if (tmscpaddr == 0)
			tmscpaddr = (struct tmscpdevice *)ubamem(io->i_adapt,
								 tmscpstd[0]);
		if (tmscp_ubaddr == 0) {
			ctmscpbuf = *io;
...
#define STEP3GOOD TMSCP_STEP4
		 ...
		 * Init cmd & rsp area
		 */
		tmscp.tmscp_ca.ca_cmddsc[0] =
		    (long)&tmscp_ubaddr->tmscp_cmd.mscp_cmdref;
		tmscp.tmscp_cmd.mscp_dscptr = tmscp.tmscp_ca.ca_cmddsc;
		tmscp.tmscp_cmd.mscp_header.tmscp_vcid = 1; /* for tape */

		tmscp.tmscp_ca.ca_rspdsc[0] =
		    (long)&tmscp_ubaddr->tmscp_rsp.mscp_cmdref;
 ...
 */
tmscpclose(io) register struct iob *io;
{
 ...
 * the command.
 */
struct mscp *tmscpcmd(op, mod) int op, mod; /* opcode and modifier (usu 0) */
{
 ...
 */
tmscpstrategy(io, func) register struct iob *io;
int func;
{
	register struct mscp *mp;
	int ubinfo;
