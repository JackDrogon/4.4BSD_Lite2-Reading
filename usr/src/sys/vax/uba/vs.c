...
u_short vsstd[] = {0};
struct uba_driver vsdriver = {vsprobe, 0,       vsattach, 0, vsstd,
			      "vs",    vsdinfo, 0,	0};
...
vsprobe(reg) caddr_t reg;
{
	...
	vsaddr->vs_csr0 =
	    cvec >> 2;	 /* Save the vector for use on next device */
...
vsattach(uip) struct uba_device *uip;
{
...
int flag;
{
	...
	printM("vsopen csr0=%x, csr1=%x, csr2=%x, csr3=%x, csr4=%x, csr5=%x, "
	       "csr6=%x, csr7=%x\n",
	       vsaddr->vs_csr0, vsaddr->vs_csr1, vsaddr->vs_csr2,
	       vsaddr->vs_csr3, vsaddr->vs_csr4, vsaddr->vs_csr5,
	       vsaddr->vs_csr6, vsaddr->vs_csr7);
	...
	vsBuffnpages =
	    (((int)vsb & PGOFSET) + (NBPG - 1) + sizeof(struct vsBuffArea)) >>
	    PGSHIFT;
	...
	vsb->vsioa.reloc =
	    (int)(vsp->offset.fparm_all + UBAI_ADDR(vsp->vsBuff_ubinfo));
	return (0);
}

vsclose(dev) dev_t dev;
{
	...
	vsaddr = (struct vsdevice *)uip->ui_addr;
	printM("vsclose csr0=%x, csr1=%x, csr2=%x, csr3=%x, csr4=%x, csr5=%x, "
	       "csr6=%x, csr7=%x\n",
	       vsaddr->vs_csr0, vsaddr->vs_csr1, vsaddr->vs_csr2,
	       vsaddr->vs_csr3, vsaddr->vs_csr4, vsaddr->vs_csr5,
	       vsaddr->vs_csr6, vsaddr->vs_csr7);
...
#ifdef notdef
	/* unmap io regs into user address space (assume they don't cross a
	 * page) */
	unmaptouser(vsaddr);
...
vsread(dev, uio) dev_t dev;
struct uio *uio;
{
	return (-1);
}

vswrite(dev, uio) dev_t dev;
struct uio *uio;
{
	return (-1);
}
...
/*ARGSUSED*/
vsioctl(dev, cmd, addr, flag) dev_t dev;
register caddr_t addr;
{
	...
	case VSIOWAITGO:
		...
		if ((ret = vsb->vsioa.status) == 0) {
			vsp->vs_nextgo.fparm_all =
			    ((struct vs_fparm *)addr)->fparm_all;
			do {
			...
			vsaddr->vs_csr0 |=
			    (VS_IE | (VS_SEND << VS_FCSHIFT) | VS_GO);
		}
		splx(s);
		if (error)
			return (error);
	...
	case VSIOUSERWAIT:
		...
		while (vsb->vsioa.status == 0) {
			error =
			    tsleep((caddr_t)vsp, VSWAITPRI | PCATCH, devio, 0);
		}
		splx(s);
		return (error);
...
vsintr(dev) dev_t dev;
{
	...
	vsb = &vsBuff[VSUNIT(dev)];
#ifdef notdef
	printM("vsintr csr0=%x, csr1=%x, csr2=%x, csr3=%x, csr4=%x, csr5=%x, "
	       "csr6=%x, csr7=%x\n",
	       vsaddr->vs_csr0, vsaddr->vs_csr1, vsaddr->vs_csr2,
	       vsaddr->vs_csr3, vsaddr->vs_csr4, vsaddr->vs_csr5,
	       vsaddr->vs_csr6, vsaddr->vs_csr7);
	...
	if (vsp->csr.csr_linkTran) {
		...
		if (vsp->csr.csr_linkAvail == vsp->linkAvail) { /* flash */
			vsp->stats.flashes++;
			printI("flash\n");
		} else if (!vsp->csr.csr_linkAvail &&
			   vsp->linkAvail) { /* on -> off */
			vsp->stats.douses++;
			printI("douse\n");
			vsp->inited = FALSE;
...
vsInitFiber(dev) dev_t dev;
{
	struct vsdevice *vsaddr =
	    (struct vsdevice *)vsdinfo[VSUNIT(dev)]->ui_addr;
	register struct vs_softc *vsp = &vs_softc[VSUNIT(dev)];
	int s, error;
...
int retry;
{
	...
	while (1) {
		...
		error = tsleep((caddr_t)vsp, VSWAITPRI | PCATCH, devwait,
			       retry ? 10 * hz : 0);
		splx(s);
...
vsError(vsp) register struct vs_softc *vsp;
{
	if (vsp->irr.intr_error) {
