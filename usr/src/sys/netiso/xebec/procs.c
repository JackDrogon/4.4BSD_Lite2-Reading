...
end_events()
{
	...
	ENDDEBUG
	/* NOSTRICT */
	Statelist = (struct Stateent **)Malloc((Nstates + 1) *
					       sizeof(struct Statent *));
	/* NOSTRICT */
	Predlist = (struct Predicate **)Malloc(
	    (((Nevents) << Eventshift) + Nstates) * sizeof(struct Predicate *));
...
int acttable(f, actstring) char *actstring;
FILE *f;
{
	static Actindex = 0;
	extern FILE *astringfile;
...
statetable(string, oldstate, newstate, action, event) char *string;
int action;
struct Object *oldstate, *newstate, *event;
{
	...
	fprintf(OUT, "statetable(%s, %s,%s, 0x%x)\n", string,
		oldstate->obj_name, newstate->obj_name, action);
	ENDDEBUG
	...
	(void)predtable(oldstate, event, string, action,
			(newstate->obj_number) * different);
	IFDEBUG(a)
	fprintf(OUT, "EXIT statetable\n");
	ENDDEBUG
}

stateentry(index, oldstate, newstate, action) int index, action;
int oldstate, newstate;
{
	...
	IFDEBUG(a)
	fprintf(OUT,
		"stateentry(0x%x,0x%x,0x%x,0x%x) Statelist@0x%x, val 0x%x\n",
...
int predtable(os, oe, str, action, newstate) struct Object *os, *oe;
char *str;
int action, newstate;
{
	...
	Ntrans++;
	IFDEBUG(g)
	fprintf(stdout, "PREDTAB: s %5s;  e %5s\n",
		os->obj_kind == OBJ_SET ? "SET" : "item",
		oe->obj_kind == OBJ_SET ? "SET" : "item");
	ENDDEBUG
	if (os->obj_kind == OBJ_SET)
		s = os->obj_members;
	else
		s = os;
	if (oe->obj_kind == OBJ_SET)
		firste = oe->obj_members;
	else
		firste = oe;
	if (newstate) {
		fprintf(statevalfile, "{0x%x,0x%x},\n", newstate, action);
	...
	while (s) {
		...
		while (e) {
			...
			if (!str /* DEFAULT PREDICATE */) {
				Ndefpred++;
				IFDEBUG(g)
				fprintf(stdout, "DEFAULT pred state 0x%x, "
						"event 0x%x, Index 0x%x\n",
					state, event, Index);
				fflush(stdout);
				ENDDEBUG
			} else
...
/* put at END of list */
#ifndef LINT
			IFDEBUG(g)
			fprintf(stdout, "predicate for event 0x%x, state 0x%x "
					"is 0x%x, %s\n",
			...
			*q = p;
			...
			IFDEBUG(g)
			fprintf(stdout, "predtable index 0x%x, transno %d, E "
					"0x%x, S 0x%x\n",
				Index, transno, e, s);
			ENDDEBUG
...
dump_predtable(f) FILE *f;
{
...
#ifdef notdef
	fprintf(stdout, " Xebec used %8d bytes of storage, wasted %8d bytes\n",
		bytesmalloced, byteswasted);
#endif notdef
	fprintf(stdout, " %8d states\n %8d events\n %8d transitions\n", Nstates,
		Nevents, Ntrans);
	fprintf(stdout, " %8d predicates\n %8d default predicates used\n",
		Npred, Ndefpred);
	fprintf(stdout, " %8d null actions\n", Nnulla);
	...
	for (e = 0; e < Nevents; e++) {
		for (s = 0; s < Nstates; s++) {
			...
			IFDEBUG(g)
			fprintf(stdout, "loop: e 0x%x s 0x%x hadapred 0x%x "
					"dindex 0x%x for trans 0x%x\n",
				e, s, hadapred, defaultindex, defaultItrans);
			ENDDEBUG
			if (hadapred) {
				/* put a -1 in the array  - Predlist is
				 * temporary storage */
				Predlist[(e << Eventshift) + s] =
				    (struct Predicate *)(-1);
			} else {
				/* put defaultindex in the array */
				/* if defaultindex is zero, then the driver will
				 * cause an erroraction (same as if no default
				 * were given and none of the predicates were
				 * true;
				 * also same as if no preds or defaults were
				 * given
				 * for this combo)
				 */
				Predlist[(e << Eventshift) + s] =
				    (struct Predicate *)(defaultindex);
			}
		}
	}
...
#ifdef notdef
dump_pentry(event, state) int event, state;
{
	register struct Predicate *p, **q;
...
#ifndef LINT
		IFDEBUG(a)
		fprintf(OUT, "dump_pentry for event 0x%x, state 0x%x is 0x%x\n",
			event, state, p);
		ENDDEBUG
#endif LINT
		q = &p->p_next;
