...
struct Object *lookup(type, name) unsigned char type;
char *name;
{
...
static insert(o) struct Object *o;
{
	...
	if (o->obj_type == STATESET) {
		...
		if (o->obj_struc) {
			...
			fprintf(Efile, "struct %s %s%s;\n\n", o->obj_struc,
				EV_PREFIX, o->obj_name);
		}
...
struct Object *defineset(type, adr, keep) unsigned char type;
char *adr;
int keep;
{
	struct Object *onew;
...
defineitem(type, adr, struc) unsigned char type;
char *adr;
char *struc;
{
	struct Object *onew;
	...
	if (onew = lookup(type, adr)) {
		fprintf(stderr, "Internal error at defineitem: trying to "
				"redefine obj type 0x%x, adr %s\n",
			type, adr);
		exit(-1);
	} else {
		onew = (struct Object *)Malloc(sizeof(struct Object));
...
char *adr;
{
	...
	if (oold == NULL) {
		...
		fprintf(stderr, "Warning at line %d: set definition of %s "
				"causes definition of\n",
			lineno, OBJ_NAME(o));
...
AddCurrentEventName(x) register char **x;
{
...
dumptree(o, i) register struct Object *o;
int i;
{
	register int j;
...
dump_trans(pred, oldstate, newstate, action, event) struct Object *oldstate,
    *newstate, *event;
char *pred, *action;
{
	extern int transno;
...
#define dumpit(x)                                                              \
	if ((x)->obj_kind == OBJ_SET) {                                        \
		...
		fprintf(stdout, "[ ");                                         \
		while (o) {                                                    \
			fprintf(stdout, "%s ", o->obj_name);                   \
			o = o->obj_members;                                    \
		}                                                              \
		fprintf(stdout, " ] ");                                        \
	} else {                                                               \
		fprintf(stdout, "%s ", (x)->obj_name);                         \
	}
