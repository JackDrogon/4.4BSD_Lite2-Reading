...
int chtype[128] =
    {
	/*	null,	soh ^a,	stx ^b	etx ^c	eot ^d	enq
	   ^e	ack ^f	bel ^g	*/
	EOFILE,
	UNUSED, UNUSED, UNUSED, UNUSED, UNUSED, UNUSED, UNUSED,
	/*	bs ^h	ht ^i	lf ^j	vt ^k	ff ^l	cr ^m
	   so ^n	si ^o	*/
	UNUSED,
	IGNORE, IGNORE, UNUSED, IGNORE, IGNORE, UNUSED, UNUSED,
	/*	dle ^p	dc1 ^q	dc2 ^r	dc3 ^s	dc4 ^t	nak
	   ^u	syn ^v	etb ^w	*/
	UNUSED,
	UNUSED, UNUSED, UNUSED, EOFILE, UNUSED, UNUSED, UNUSED,
	/*	can ^x	em ^y	sub ^z	esc ^]	fs ^\ 	gs ^}
	   rs ^`	us ^/	*/
	UNUSED,
	UNUSED, UNUSED, UNUSED, UNUSED, UNUSED, UNUSED, UNUSED,
	...
	/*			!		"		#		$
	   %		&		'		*/
	IGNORE,
	UNUSED, OPCHAR, UNUSED, OPCHAR, UNUSED, OPCHAR, OPCHAR,
	/*	(		)		*		+		,
	   -		.		/		*/
	OPCHAR,
	OPCHAR, OPCHAR, OPCHAR, OPCHAR, OPCHAR, OPCHAR, OPCHAR,
	/*	0		1		2		3		4
	   5		6		7		*/
	DIGITS,
	...
	/*	8		9		:		;		<
	   =		>		?		*/
	DIGITS,
	DIGITS, OPCHAR, OPCHAR, OPCHAR, OPCHAR, OPCHAR, OPCHAR,
	...
	LETTER,
	LETTER, LETTER, OPCHAR, UNUSED, OPCHAR, OPCHAR, LETTER,
	...
	LETTER,
	LETTER, LETTER, OPCHAR, UNUSED, OPCHAR, UNUSED, UNUSED};
    ...
    c;
{
	...
	while (nested) {
		if (ch == '\0') {
			fprintf(
			    stderr,
			    "Eof inside of a string, delims= %c,%c, nesting %d",
			    c, o, nested);
			Exit(-1);
			/* notreached */
		} else if (ch == '$') {
			...
			ch = getch();
			if ((ch != o) && (ch != c) && (ch != '$')) {
				/* may need to handle case where \ is last char
				 * in file... */
				/* don't treat is as escape; not open or close
				 * so
	...
	check:
		if (ch == o)
			nested += allow_nesting;
		else if (ch == c)
			nested--;
		if ((buffptr - buffer[currentbuf]) > LINELEN) {
			fprintf(stderr, "%s too long.\n",
				(o == '{') ? "Action" : "Predicate"); /*}*/
			fprintf(stderr, "buffptr, currentbuf 0x%x, 0x%x\n",
				buffptr, currentbuf);
			Exit(-1);
		}
		IFDEBUG(S)
...
again:
	...
	case UNUSED:
		fprintf(stderr, "Illegal character in input - 0x%x ignored.",
			ch);
		ch = getch();
		goto again;
	...
	case OPCHAR:
		...
		case '/':
			...
			if (c != '*') {
				fprintf(
				    stderr,
				    "Syntax error : character(0x%x) ignored",
				    ch);
				ch = c;
				goto again;
			} else {
				...
				while (state) {
					...
					whatchar =
					    (c == '*') ? 0 : (c == '/' ? 1 : 2);
					IFDEBUG(S)
					fprintf(stdout, "comment: whatchar = "
							"%d, c = 0x%x,%c, "
							"oldstate=%d",
						whatchar, c, c, state);
					ENDDEBUG
				...
				if (state) {
					fprintf(stderr, "Syntax error: end of "
							"file inside a "
							"comment");
					Exit(-1);
				} else
