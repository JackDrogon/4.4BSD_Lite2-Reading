	...
	do {
		...
		if (llepsilon[parseaction]) {
			...
			if (llepsilonok(t->llterm)) {
				...
				llpushprod(
				    parseaction); /* push rhs of production */
			} else {
				llparsererror(t);
				havetoken = FALSE;
			}
    ...
    short prod;
{
	...
	printf("llpushprod(%d) llstackptr=0x%x(%d), length = 0x%x(%d)\n", prod,
	       llstackptr, llstackptr, length, length);
	/*
	dump_parse_stack();
	*/
	ENDDEBUG
	...
	if (llparsestack[llstackptr] <= LLMINACTION) {
		(llparsestack[llstackptr])--; /* if there's already one there,
						 don't
							       put another on;
						 just let it represent all of
							       the adjacent
						 markers */
	} else {
		llstackptr++;
...
llparsererror(token) LLtoken *token;
{
	IFDEBUG(L)
...
llgettoken(token) LLtoken *token;
{
	llscan(token);
...
llfinprod()
{
	...
	llattrdesc[lldescindex - 1]
	    .llaindex++; /* lhs-of-prod.attr stays on
...
dump_parse_stack()
{
	...
	for (ind = llstackptr; ind >= 0; ind--) {
		printf("%d\t%d\t%s\n", ind, llparsestack[ind],
		       llparsestack[ind] < 0 ? "Action symbol"
					     : llstrings[llparsestack[ind]]);
	}
}
