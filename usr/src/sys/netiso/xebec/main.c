...
openfiles(proto) register char *proto;
{
	register char *junk;
	...
	if (argc > 2)
		while (i < argc) {
			...
			case 'X':
				...
				astringfile_name =
				    (char *)strcpy(astringfile_name, name);
#ifdef LINT
				name =
#endif LINT
			...
			case 'E':
				...
				eventfile_h_name =
				    (char *)strcpy(eventfile_h_name, name);
#ifdef LINT
				name =
#endif LINT
			...
			case 'I':
				...
				statevalfile_name =
				    (char *)strcpy(statevalfile_name, name);
#ifdef LINT
				name =
#endif LINT
			...
			case 'S':
				...
				statefile_name =
				    (char *)strcpy(statefile_name, name);
#ifdef LINT
				name =
#endif LINT
...
Exit(n)
{
	...
	if (transno)
		fprintf(stderr, "Transition number %d\n", transno);
	(void)fflush(stdout);
...
FakeFilename(outfile, name, l) FILE *outfile;
char *name;
int l;
{
	/*
