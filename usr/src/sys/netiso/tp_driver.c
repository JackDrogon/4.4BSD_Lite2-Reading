...
int tp_emit(), tp_goodack(), tp_goodXack(), tp_stash();
void tp_indicate(), tp_getoptions(), tp_soisdisconnecting(),
    tp_soisdisconnected(), tp_recycle_tsuffix(),
#ifdef TP_DEBUG_TIMERS
    tp_etimeout(), tp_euntimeout(), tp_ctimeout(), tp_cuntimeout(),
    tp_ctimeout_MIN(),
#endif
    tp_freeref(), tp_detach(), tp0_stash(), tp0_send(), tp_netcmd(), tp_send();
...
_XEBEC_PG int _Xebec_action(a, e, p) int a;
struct tp_event *e;
tp_PCB_ *p;
{
	switch (a) {
	...
	case 0x3: {
		...
		(void)tp_goodack(p, e->ev_union.EV_AK_TPDU.e_cdt,
				 e->ev_union.EV_AK_TPDU.e_seq,
				 e->ev_union.EV_AK_TPDU.e_subseq);
	...
	case 0xa: {
		...
		if (error = tp_emit(CR_TPDU_type, p, 0, 0, data))
			return error; /* driver WON'T change state; will return
					 error */
	...
	case 0xb: {
		...
		if (p->tp_state == TP_OPEN)
			...
			if (e->ev_union.EV_DR_TPDU.e_reason !=
				(E_TP_NO_SESSION ^ TP_ERROR_MASK) &&
			    e->ev_union.EV_DR_TPDU.e_reason !=
				(E_TP_NO_CR_ON_NC ^ TP_ERROR_MASK) &&
			    e->ev_union.EV_DR_TPDU.e_reason !=
				(E_TP_REF_OVERFLOW ^ TP_ERROR_MASK))
	...
	case 0x11: { /* don't ask me why we have to do this - spec says so */
	...
	case 0x15: {
		...
		if ((p->tp_rx_strat & TPRX_FASTSTART) &&
			...
			p->tp_cong_win =
			    e->ev_union.EV_CC_TPDU.e_cdt * p->tp_l_tpdusize;
		tp_getoptions(p);
		tp_cuntimeout(p, TM_retrans);
		if (p->tp_ucddata) {
			IFDEBUG(D_CONN)
	...
	case 0x1b: {
		...
		if (doack)
			...
			tp_ctimeout_MIN(p, TM_sendack,
					(int)p->tp_sendack_ticks);
	...
	case 0x1c: {
		...
		tptrace(TPPTmisc, "NIW seq rcvnxt lcredit ",
			e->ev_union.EV_DT_TPDU.e_seq, p->tp_rcvnxt,
			p->tp_lcredit, 0);
		ENDTRACE
		IncStat(ts_dt_niw);
	...
	case 0x20: {
		...
		tp_ctimeout(p, TM_inact, (int)p->tp_inact_ticks);
		/* don't send an xack because the xak gives "last one received",
		 * not
	...
	case 0x22: {
		...
		if (p->tp_state != TP_CLOSING) {
			tp_soisdisconnecting(p->tp_sock);
			data = MCPY(p->tp_ucddata, M_NOWAIT);
			(void)tp_emit(DR_TPDU_type, p, 0, E_TP_NORMAL_DISC,
				      data);
			p->tp_retrans = p->tp_Nretrans;
			tp_ctimeout(p, TM_retrans, (int)p->tp_dr_ticks);
		}
	...
	case 0x24: {
		...
		if (trick_hc)
			return tp_emit(DR_TPDU_type, p, 0,
				       e->ev_union.EV_T_DISC_req.e_reason,
				       data);
	} break;
	case 0x25: {
		int error;
	...
	case 0x28: {
		...
		if (p->tp_Xsnd.sb_mb) {
			struct mbuf *m =
			    m_copy(p->tp_Xsnd.sb_mb, 0, (int)p->tp_Xsnd.sb_cc);
			int shift;
			...
			tp_ctimeout(p, TM_retrans, ((int)p->tp_dt_ticks)
						       << shift);
		}
	} break;
	case 0x29: {
	...
	case 0x2d: {
		if (p->tp_class != TP_CLASS_0) {
			...
			if (e->ev_number == CC_TPDU)
				(void)tp_emit(AK_TPDU_type, p, p->tp_rcvnxt, 0,
					      MNULL);
		}
	...
	case 0x2f: {
		...
		if (p->tp_Xsnd.sb_mb) {
			struct mbuf *m =
			    m_copy(p->tp_Xsnd.sb_mb, 0, (int)p->tp_Xsnd.sb_cc);
			/* m_copy doesn't preserve the m_xlink field, but at
			 * this pt.
			 * that doesn't matter
			 */
			...
			ENDDEBUG
			error = tp_emit(XPD_TPDU_type, p, p->tp_Xuna, 1, m);
			p->tp_retrans = p->tp_Nretrans;
	...
	case 0x31: {
		...
		tptrace(TPPTmisc, "BOGUS ACK fcc_present, tp_r_subseq e_subseq",
			e->ev_union.EV_AK_TPDU.e_fcc_present, p->tp_r_subseq,
		...
		if (p->tp_class != TP_CLASS_0) {
			...
			if (!e->ev_union.EV_AK_TPDU.e_fcc_present) {
				...
				(void)tp_emit(AK_TPDU_type, p, p->tp_rcvnxt, 1,
					      MNULL);
			}
			tp_ctimeout(p, TM_inact, (int)p->tp_inact_ticks);
		}
	...
	case 0x33: {
		IFTRACE(D_ACKRECV)
		tptrace(TPPTmisc, "BOGUS XACK eventtype ", e->ev_number, 0, 0,
			0);
		ENDTRACE
		if (p->tp_class != TP_CLASS_0) {
			tp_ctimeout(p, TM_inact, (int)p->tp_inact_ticks);
	...
	case 0x36: {
		if (trick_hc) {
			 ...
			 * A reasonable amount is 2 packets, unless the max
			 * window
			 * is only 1 or 2 packets, in which case we
			 * should send an ack for any advance in the upper
			 * window edge.
			 */
			LOCAL_CREDIT(p);
			...
			if (SEQ_GT(p, ack_thresh, p->tp_sent_uwe)) {
				...
				p->tp_flags &= ~TPF_DELACK;
				return tp_emit(AK_TPDU_type, p, p->tp_rcvnxt, 0,
					       MNULL);
			}
		}
	} break;
...
_XEBEC_PG int _Xebec_index(e, p) struct tp_event *e;
tp_PCB_ *p;
{
	...
	case 0xb2:
		if (p->tp_class == TP_CLASS_0)
			...
			return 0x0;
	case 0xb4:
		if (tp_goodack(p, e->ev_union.EV_AK_TPDU.e_cdt,
			       e->ev_union.EV_AK_TPDU.e_seq,
			       e->ev_union.EV_AK_TPDU.e_subseq))
			return 0x3a;
		else
			return 0x3b;
	case 0xc3:
		if (IN_RWINDOW(p, e->ev_union.EV_DT_TPDU.e_seq, p->tp_rcvnxt,
			       SEQ(p, p->tp_rcvnxt + p->tp_lcredit)))
			return 0x21;
		else
			return 0x24;
	case 0xc4:
	...
	case 0x188:
		if (p->tp_class == TP_CLASS_0)
			return 0xf;
		else if (tp_emit(CC_TPDU_type, p, 0, 0,
				 MCPY(p->tp_ucddata, M_NOWAIT)) == 0)
			return 0x10;
		else
			return 0x11;
	default:
...
tp_driver(p, e) register tp_PCB_ *p;
register struct tp_event *e;
{
	...
	IFTRACE(D_DRIVER)
	tptrace(DRIVERTRACE, a->a_newstate, p->tp_state, e->ev_number,
		a->a_action, 0);
	ENDTRACE
	if (error == 0)
