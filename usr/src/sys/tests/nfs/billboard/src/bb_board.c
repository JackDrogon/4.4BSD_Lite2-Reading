	...
	if ((board_file = fopen(BB_BOARD_FILE, "r")) != NULL) {
		for (i = 0; i < num_imps; i++) {
			for (j = 0; j < num_imps; j++) {
				if ((board[i][j] = getw(board_file)) ==
				    (u_char)EOF) {
					fprintf(stderr, "ERROR: Board data "
							"file incomplete.\n");
					return BB_FAILURE;
				}
			}
	...
	for (i = 0; i < num_imps; i++) {
		for (j = 0; j < num_imps; j++) {
			if (putw((int)BB_BOARD_UNSET, board_file) == 1) {
				fprintf(stderr,
					"FAILED writting new board file.\n");
				fclose(board_file);
				return BB_FAILURE;
			}
		}
...
**									**
*************************************************************************/
void bb_board_tag(p_in, p_out, set)
...
**									**
*************************************************************************/
int bb_get_clients(id, list) BB_id id; /* Id of the client.		*/
...
**									**
*************************************************************************/
int bb_get_servers(id, list) BB_id id; /* Id of the client.		*/
BB_row list;			       /* List of all the servers.	*/
{
	int index; /* Index of the client.		*/
...
*************************************************************************/
int bb_board_update(
    client, server) int client; /* The index of the client to update.	*/
int server;			/* The index of the server to update.	*/
{
	...
	*/
	if ((board_file = fopen(BB_BOARD_FILE, "r+")) == NULL) {
		fprintf(stderr,
			"ERROR: Could not open board file for update.\n");
		return BB_FAILURE;
	}
