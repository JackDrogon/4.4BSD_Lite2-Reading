 ...
 */
/* usage message */
char *_Usage = "Usage: %s	[-s|-u client_idenitfier server_identifier]\n\
 ...
 ******************************************************************************
 */
static int _cmd_line_option(argc, argv) int argc;
char *argv[];
{
	int option;
	BB_set_in bb_set_in;
	...
	bb_set_in.client[BB_ID_NAME_LEN - 1] =
	    bb_set_in.server[BB_ID_NAME_LEN - 1] = '\0';
	...
	if ((option = getopt(argc, argv, "s:u:a:b:c:d:p:")) != -1) {
		 ...
		 */
		case 's':
			strncpy(bb_set_in.client, optarg, BB_ID_NAME_LEN - 1);
		 ...
		 */
		case 'a':
			strncpy(bb_list_in.id, optarg, BB_ID_NAME_LEN - 1);
 ...
 ******************************************************************************
 */
static bool_t _bb_get_passwd(bb_passwd_in_p) BB_passwd_in *bb_passwd_in_p;
{
	char buffer[BB_MAX_LINE_LEN];
