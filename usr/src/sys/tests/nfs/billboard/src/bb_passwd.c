	...
	*/
	if ((passwd_file = fopen(BB_PASSWD_FILE, "r")) != NULL) {
		while (fread(passwds[i], BB_PASSWD_LEN, 1, passwd_file) != 0) {
			...
			if (i > num_imps) {
				fprintf(stderr,
					"ERROR: Too many passwords in file.\n");
				fclose(passwd_file);
				return BB_FAILURE;
			}
		}
...
*************************************************************************/
int bb_check_passwd(
    id, passwd) BB_id id; /* The identifier of the passwd owner.	*/
...
**									**
*************************************************************************/
int bb_passwd_update(
    index) int index; /* The index of the changed password.	*/
{
	int num_imps;      /* The number of implementations.	*/
	FILE *passwd_file; /* Pointer to the passwd file.		*/
	...
	*/
	if ((passwd_file = fopen(BB_PASSWD_FILE, "r+")) == NULL) {
		fprintf(stderr,
			"ERROR: Could not open password file for update.\n");
		return BB_FAILURE;
	}
