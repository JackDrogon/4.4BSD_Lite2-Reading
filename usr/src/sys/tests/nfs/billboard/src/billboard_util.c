 ...
 ******************************************************************************
 */
void bb_call_set_unset(flag, bb_set_in_p) int flag;
BB_set_in *bb_set_in_p;
{
	...
	if (flag == BB_SET)
		...
		bb_set_out_p =
		    (BB_set_out *)bb_call_rpc(bb_unset_1, bb_set_in_p);
	else
		return;
	...
	if (bb_set_out_p->status == BB_BAD_PASSWD) {
		...
		strncpy(bb_set_in_p->passwd,
			crypt(getpass("Password:"), _bb_salt),
			BB_PASSWD_LEN - 1);
		/*
		...
		if (flag == BB_SET)
			bb_set_out_p =
			    (BB_set_out *)bb_call_rpc(bb_set_1, bb_set_in_p);
		else
			bb_set_out_p =
			    (BB_set_out *)bb_call_rpc(bb_unset_1, bb_set_in_p);
 ...
 ******************************************************************************
 */
void bb_set_unset_print_result(flag, bb_set_in_p, bb_set_out_p) int flag;
BB_set_in *bb_set_in_p;
BB_set_out *bb_set_out_p;
{
	...
	case BB_SUCCESS:
		if (flag == BB_SET)
			printf("The following client and server implementation "
			       "is tested succesfully:\n");
		else
			printf("The following client and server implementation "
			       "is set as being untested:\n");
		printf("Client:\n");
	...
	case BB_BAD_SERVER:
		...
		break;
	case BB_ALREADY_SET:
		fprintf(stderr, "Client %s against server %s has already been "
				"set as tested.\n",
	...
	case BB_BAD_PHASE:
		fprintf(stderr, "You have to complete testing against a Sun "
 ...
 ******************************************************************************
 */
void bb_list(flag, bb_list_in_p) int flag;
 ...
 ******************************************************************************
 */
void bb_list_print_result(flag, bb_list_in_p, bb_list_out_p) int flag;
BB_list_in *bb_list_in_p;
BB_list_out *bb_list_out_p;
{
	int i;
	...
	case BB_SUCCESS:
		...
		if ((data_len = bb_list_out_p->data.data_len) > 0) {
			switch (flag) {
			case BB_ALIST:
				printf("List of servers successfully tested "
				       "against client %s:\n",
				       bb_list_in_p->id);
				break;
			case BB_BLIST:
				printf("List of servers not tested against "
				       "client %s:\n",
				       bb_list_in_p->id);
				break;
			case BB_CLIST:
				printf("List of clients successfully tested "
				       "against server %s:\n",
				       bb_list_in_p->id);
				break;
			case BB_DLIST:
				printf("List of clients not tested against "
				       "server %s:\n",
				       bb_list_in_p->id);
				break;
			}
			...
			switch (flag) {
			case BB_ALIST:
				printf("No servers successfully tested against "
				       "client %s.\n",
				       bb_list_in_p->id);
				break;
			case BB_BLIST:
				printf("All servers are successfully tested "
				       "against client %s.\n",
				       bb_list_in_p->id);
				break;
			case BB_CLIST:
				printf("No clients successfully tested against "
				       "server %s.\n",
				       bb_list_in_p->id);
				break;
			case BB_DLIST:
				printf("All clients are successfully tested "
				       "against server %s.\n",
				       bb_list_in_p->id);
				break;
			}
 ...
 ******************************************************************************
 */
void bb_change_passwd(bb_passwd_in_p) BB_passwd_in *bb_passwd_in_p;
{
	int *status_p;
 ...
 ******************************************************************************
 */
void bb_print_vendor_info(bb_vendor_p) BB_vendor *bb_vendor_p;
{
	printf("\tBooth: %d\n", bb_vendor_p->booth);
	printf("\tCompany: %s\n", bb_vendor_p->company);
