			...
			*/
			if (bb_get_codata(i, &codata) == BB_SUCCESS) {
				...
				strncpy(vendors[j].company, codata.company,
					BB_COMPANY_NAME_LEN);
				strncpy(vendors[j].imp, codata.imp,
					BB_IMP_NAME_LEN);
				strncpy(vendors[j].id, codata.id,
					BB_ID_NAME_LEN);
				j++;
			}
		}
