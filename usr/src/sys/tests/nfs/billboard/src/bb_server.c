...
main()
{
	...
	if (!svc_register(transp, BILLBOARD_PROG, BILLBOARD_VERS,
			  billboard_prog_1, IPPROTO_UDP)) {
		fprintf(stderr, "unable to register (BILLBOARD_PROG, "
				"BILLBOARD_VERS, udp).");
		exit(1);
	}
...
void bb_get_ip(ip) BB_ip ip; /* Pointer to an ip for output.		*/
{
...
static void billboard_prog_1(rqstp, transp) struct svc_req *rqstp;
register SVCXPRT *transp;
{
	union {
