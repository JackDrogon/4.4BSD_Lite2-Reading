	...
	if ((phase_file = fopen(BB_PHASE_FILE, "r")) != NULL) {
		for (i = 0; i < num_imps; i++) {
			if ((phases[i] = getw(phase_file)) == (u_char)EOF) {
				fprintf(stderr,
					"ERROR: Phase data file incomplete.\n");
				return BB_FAILURE;
			}
		}
...
*************************************************************************/
int bb_phase_update(
    client) int client; /* The client with the change of phase.	*/
{
	int num_imps;     /* The number of implementations.	*/
	FILE *phase_file; /* Pointer to the phase file.		*/
	...
	*/
	if ((phase_file = fopen(BB_PHASE_FILE, "r+")) == NULL) {
		fprintf(stderr,
			"ERROR: Could not open phase file for update.\n");
		return BB_FAILURE;
	}
...
*************************************************************************/
int bb_set_phase(
    client, phase) BB_id client; /* The name of the client to change.	*/
...
**									**
*************************************************************************/
int bb_phase_ok(client) BB_id client; /* The client identifier.		*/
{
	int client_id; /* The index of the client.		*/
