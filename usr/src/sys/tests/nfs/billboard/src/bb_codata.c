...
int bb_get_codata(
    index, p_codata) int index; /* The index of the data to retrieve.	*/
...
int bb_get_imp_cnt() { return co_cnt; }
	...
	while ((status = bb_read_1_codata(&cdata)) != BB_END_OF_FILE) {
		...
		*/
		if (status != BB_SUCCESS) {
			fprintf(stderr,
				"ABORTING: Can't read company data file.\n");
			return BB_FAILURE;
		}
		...
		*/
		if ((index = bb_put_codata(&cdata)) == BB_FAILURE) {
			fprintf(stderr,
				"ABORTING: Not enough company data space.\n");
			return BB_FAILURE;
		}
		...
		*/
		if (bb_put_hash(codata[index].id, index) != BB_SUCCESS) {
			fprintf(stderr,
				"ABORTING: Unable to hash company: %s.\n",
				cdata.company);
			return BB_FAILURE;
		}
	}
	...
	*/
	if (file == NULL) {
		if ((file = fopen(BB_CODATA_FILE, "r")) == NULL) {
			fprintf(stderr,
				"FAILED opening the company data file '%s'.\n",
				BB_CODATA_FILE);
			return BB_FAILURE;
		}
	...
	while (done != TRUE) {
		...
		case BB_IP_DESIGNATOR:
			bb_get_ip_lines(&line[BB_DES_START], &ip_count,
					&ip_index);
			break;
		...
		default:
			if (line[0] != '\n') {
				fprintf(
				    stderr,
				    "ERROR: Invalid field designator '%s'.\n",
				    line);
			}
			break;
		}
