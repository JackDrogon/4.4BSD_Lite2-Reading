...
bool_t eachresult() { return (1); }

/*
 * XDR remote call arguments
 * written for XDR_ENCODE direction only
 */
static bool_t xdr_rmtcall_args(xdrs, cap) register XDR *xdrs;
 ...
 */
static bool_t xdr_rmtcallres(xdrs, crp) register XDR *xdrs;
...
static int getbroadcastnets(sock, buf) int sock; /* any valid socket will do */
	...
	for (i = 0, n = ifc.ifc_len / sizeof(struct ifreq); n > 0; n--, ifr++) {
		...
		if ((ifreq.ifr_flags & IFF_BROADCAST) &&
			...
			if (ioctl(sock, SIOCGIFBRDADDR, (char *)&ifreq) < 0) {
				baddr.sin_addr = inet_makeaddr(
				    inet_netof(sin->sin_addr), INADDR_ANY);
			} else {
				baddr.sin_addr =
...
#else
			baddr.sin_addr = inet_makeaddr(
			    inet_netof(sin->sin_addr.s_addr), INADDR_ANY);
#endif
			break;
		}
	}
...
enum clnt_stat clnt_broadcast(sock, prog, vers, proc, xargs, argsp, xresults,
			      resultsp, eachresult, t) int sock;
#ifdef SVR3
...
struct timeval *t;
{
	...
	if (sendto(sock, outbuf, outlen, 0, (struct socketaddr *)&baddr,
