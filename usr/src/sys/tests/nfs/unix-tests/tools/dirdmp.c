...
struct direct *
#endif
    readdir(dirp) register DIR *dirp;
{
#ifdef SVR3
...
#ifdef SVR3
			dirp->dd_size =
			    getdents(dirp->dd_fd, dirp->dd_buf, DIRBUF);
#else
			dirp->dd_size =
			    getdirentries(dirp->dd_fd, dirp->dd_buf,
					  dirp->dd_bsize, &dirp->dd_bbase);
#endif
			if (dirp->dd_size <= 0) {
				printf("EOF\n");
				return (NULL);
...
#endif
#ifdef SVR3
		printf("%5d %5ld %5d %s\n", dirp->dd_loc, dp->d_ino,
		       dp->d_reclen, dp->d_name);
#else
		printf("%5d %5d %5d %5d %s\n", dirp->dd_loc, dp->d_fileno,
